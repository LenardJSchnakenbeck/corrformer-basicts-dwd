...
from .corrformer_layers import Encoder, Decoder, EncoderLayer, DecoderLayer, DataEmbedding
...
class Corrformer(nn.Module):
    def __init__(self, configs):
        ...
        # Encoding
        self.enc_embedding = DataEmbedding(configs.enc_in, configs.d_model, configs.node_num, configs.num_spatial,
                                           configs.embed, configs.timestamp_features, configs.dropout)
        self.dec_embedding = DataEmbedding(configs.dec_in, configs.d_model, configs.node_num, configs.num_spatial,
                                           configs.embed, configs.timestamp_features, configs.dropout)
    ...
    def forward(self, inputs, spatial_inputs, inputs_timestamps, dec_inputs, dec_inputs_timestamps,
                enc_self_mask=None, dec_self_mask=None, dec_enc_mask=None):

        x_enc = inputs # B (batch_size = 1), seq_len/input_len, num_stations
        x_spatial = spatial_inputs.squeeze(0)
        x_mark_enc = inputs_timestamps
        x_dec = dec_inputs
        x_mark_dec = dec_inputs_timestamps
        ...
        enc_out = self.enc_embedding(x_enc, x_spatial, x_mark_enc)
        ...
        dec_out = self.dec_embedding(seasonal_init, x_spatial, x_mark_dec)
